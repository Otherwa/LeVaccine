...
app.post('/', async (req, res) => {
  ...
  const user_check = usersemails.findOne({ email: req.body.email })
  ...
  edurls.find({}, { _id: 0, url: 1, date: 1, description: 1 }, (err, data) => {
    if (err) {
      ...
      edurls.countDocuments({}, (err, usercount) => {
        ...
        res.render('education', { data, count: usercount })
      })
    }
  })
})
...
app.get('/api/edurls/:url&:des&:apikey', async (req, res) => {
  ...
  const url = youtube + req.params.url
  const apikey = req.params.apikey
  const description = req.params.des
  const date = new Date()

  await connect()
  const exsist = await api.findOne({ apikey }).lean()
  ...
  if (exsist != null) {
    ...
    edurl.save((err) => {
      if (err) {
        ...
        res.send({ msg: 'Ok', status: '200', url, apikey }).status(404)
      }
    })
  } else {
